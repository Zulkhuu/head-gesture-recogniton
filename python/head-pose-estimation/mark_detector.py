    ...
    def draw_all_result(self, image):
        ...
        for facebox, conf in self.detection_result:
            ...
            cv2.rectangle(image, (facebox[0], facebox[1] - label_size[1]),
                          (facebox[0] + label_size[0],facebox[1] + base_line),(0, 255, 0), cv2.FILLED)
...
class MarkDetector:
    ...
    @staticmethod
    def draw_box(image, boxes, color=(255, 255, 255)):
        """Draw square boxes on image"""
        for box in boxes:
            cv2.rectangle(image,(int(box[0]), int(box[1])),(int(box[2]), int(box[3])), color)

    @staticmethod
    def draw_crosshairs(image,bbox,color=(255, 255, 255)):
        w = bbox[2]-bbox[0]
        h = bbox[3]-bbox[1]
        # Line Property
        x_line_width = np.max([int(0.015*w),1])
        y_line_width = np.max([int(0.015*h),1])
        x_line_len   = np.max([int(0.150*w),1])
        y_line_len   = np.max([int(0.150*h),1])
        # Shift Y coordinate
        x_left   = int(bbox[0])
        x_right  = int(bbox[2])
        y_top    = int(bbox[1])
